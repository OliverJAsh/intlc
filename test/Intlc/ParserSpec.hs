...
spec = describe "parser" $ do
  describe "TAST" $ do
    it "plaintext" $ do
      runParser plaintextParser "test" "hello" `shouldBe` Right (TPlaintext "hello")

    it "interpolation" $ do
      runParser interpolationParser "test" "{name}" `shouldBe` Right (TInterpolation "name")

    it "node (interpolation)" $ do
      runParser tnodeParser "test" "{name}" `shouldBe` Right (TInterpolation "name")

    it "node (plaintext)" $ do
      runParser tnodeParser "test" "hello" `shouldBe` Right (TPlaintext "hello")

    it "TAST" $ do
      runParser tastParser "test" "hello {name}, you are {years} years old"
        `shouldBe` Right
          [ TPlaintext "hello ",
            TInterpolation "name",
            TPlaintext ", you are ",
            TInterpolation "years",
            TPlaintext " years old"
          ]
  ...
  describe "message" $ do
    ...
    describe "plural hash" $ do
      it "parses as plaintext outside of plurals" $ do
        ...
        parse msg "{x, select, y {#}}"
          `shouldParse` ( Dynamic . pure . Interpolation . Arg "x" $
      ...
      it "parses as arg inside shallow plural" $ do
        ...
        parse msg "{n, plural, one {#} other {#}}"
          `shouldParse` ( Dynamic . pure . Interpolation . Arg "n" . Plural . Cardinal $
      ...
      it "parses as nearest arg inside deep plural" $ do
        ...
        parse msg "{n, plural, one {{i, plural, one {#} other {#}}} other {#}}"
          `shouldParse` ( Dynamic . pure . Interpolation . Arg "n" . Plural . Cardinal $
                            RulePlural
                              ( pure $
                                  PluralCase
      ...
      it "parses as arg nested inside other interpolation" $ do
        let n = pure . Interpolation $ Arg "n" PluralRef
        parse msg "{n, plural, one {<f>#</f>} other {#}}"
          `shouldParse` ( Dynamic . pure . Interpolation . Arg "n" . Plural . Cardinal $
                            RulePlural
                              ( pure $
                                  PluralCase
    ...
    describe "escaping" $ do
      it "escapes non-empty contents between single quotes" $ do
        parse msg "These are not interpolations: '{word1} {word2}'"
          `shouldParse` Static "These are not interpolations: {word1} {word2}"
        parse msg "'<notATag>hello</notATag>'"
          `shouldParse` Static "<notATag>hello</notATag>"
        parse msg "a {b} '{c}' {d} e"
          `shouldParse` Dynamic (Plaintext "a " :| [Interpolation (Arg "b" String), Plaintext " {c} ", Interpolation (Arg "d" String), Plaintext " e"])
      ...
      it "escapes two single quotes as one single quote" $ do
        ...
        parse msg "a {b} ''{c}'' {d} e"
          `shouldParse` Dynamic (Plaintext "a " :| [Interpolation (Arg "b" String), Plaintext " '", Interpolation (Arg "c" String), Plaintext "' ", Interpolation (Arg "d" String), Plaintext " e"])
  ...
  describe "callback" $ do
    it "parses nested" $ do
      parse callback "<f><g>x{y}z</g></f>"
        `shouldParse` Arg "f" (Callback [Interpolation $ Arg "g" (Callback [Plaintext "x", Interpolation (Arg "y" String), Plaintext "z"])])
  ...
  describe "plural" $ do
    ...
    it "parses literal and plural cases, wildcard, and interpolation token" $ do
      parseWith (ParserState (Just "xyz")) cardinalPluralCases "=0 {foo} few {bar} other {baz #}"
        `shouldParse` Cardinal (MixedPlural (pure $ PluralCase (PluralExact "0") [Plaintext "foo"]) (pure $ PluralCase Few [Plaintext "bar"]) (PluralWildcard [Plaintext "baz ", Interpolation (Arg "xyz" PluralRef)]))
