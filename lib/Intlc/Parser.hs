...
parseDataset = parse' <=< decode'
  where
    decode' = maybeToRight FailedJsonParse . decode
...
callback = do
  ...
  where
    children = Callback . mergePlaintext <$> manyTill token (lookAhead $ string "</")
...
-- {myBool, boolean, true {blah} false {blah}}
...
selectCases = (,) <$> cases <*> optional wildcard
  where
    ...
    cases = NE.sepEndBy1 (SelectCase <$> (name <* hspace1) <*> caseBody) hspace1
...
cardinalPluralCases = fmap Cardinal . tryClassify =<< p
  where
    tryClassify = maybe empty pure . uncurry classifyCardinal
...
ordinalPluralCases = fmap Ordinal . tryClassify =<< p
  where
    tryClassify = maybe empty pure . uncurry classifyOrdinal
...
-- Hello {name}
data TNode = TPlaintext String | TInterpolation {arg :: String}
  deriving (Show, Eq)
...
type TASTParser = TParser TAST

openBraceOrEof :: TParser ()
openBraceOrEof = void (char '{') <|> eof

plaintextParser :: TParser TNode
plaintextParser = manyTill L.charLiteral (lookAhead openBraceOrEof) <&> TPlaintext

interpolationParser :: TParser TNode
interpolationParser = between (char '{') (char '}') (some letterChar) <&> TInterpolation

tnodeParser :: TParser TNode
tnodeParser = interpolationParser <|> plaintextParser

tastParser :: TParser TAST
tastParser = manyTill tnodeParser eof
